...
async function loadSymbolsFromConfig() {
  try {
    ...
    const syms = j && Array.isArray(j.symbols) ? j.symbols : [];
    return syms.map(s => String(s).toUpperCase().trim()).filter(Boolean);
  } catch (err) {
    console.error("[manual] Failed to load symbols from /api/config:", err);
    return [];
  }
}
...
// ---- bootstrap ----
(async function boot() {
  const symbols = await loadSymbolsFromConfig();
  if (!symbols.length) {
    // Fail loud: manual terminal must follow backend universe.
    const tr = document.createElement("tr");
    tr.innerHTML = `<td colspan="4" style="color: var(--app-text-muted)">No symbols loaded. Check backend /api/config.</td>`;
    tbody.appendChild(tr);
    return;
  }
  for (const sym of symbols) makeRow(sym);
})();
...
async function updateIndicator(sym, ot, ol, pdc){
  ...
  try{
    ...
    const [f, m] = await Promise.all([
      fetch(`/api/freq3?${q}`, { cache:"no-store" }).then(r=>r.json()),
      fetch(`/api/matches?${q}`, { cache:"no-store" }).then(r=>r.json()),
    ]);

    const pick  = String(f?.pick || "ABSTAIN").toUpperCase();
    const conf  = Number(f?.conf_pct || 0);
    const level = String(f?.level || "-");

    const c = m?.counts || null;
    let countsTxt = "";
    if (c){
      const B  = Number(c.BULL ?? c.bull ?? 0);
      const R  = Number(c.BEAR ?? c.bear ?? 0);
      const TR = Number(c.TR ?? c.tr ?? 0);
      const T  = Number(c.TOTAL ?? c.total ?? (B + R + TR));
      // compact chip text
      countsTxt = ` · B${B} R${R} TR${TR} T${T}`;
    }

    el.innerHTML = `
      <span class="pill ${pick.toLowerCase()}">${pick}</span>
      <span class="meta">${conf}% · ${level}${countsTxt}</span>
    `;
  }catch(e){
    console.error(e);
...
// init is handled by boot() at top (symbols loaded from /api/config)
...
function drawSuper(canvas, cone, meta){
  ...
  levels.forEach(v => {
    ...
    ctx.font = "11px system-ui, -apple-system, BlinkMacSystemFont, 'SF Pro Text', sans-serif";
  ...
  // IQR cone (p25..p75) – brighter on dark
  ctx.fillStyle = "rgba(56,189,248,0.22)";
  ctx.beginPath();
  for (let i = 0; i < n; i++) {
    const x = xAt(i), y = yAt(p75[i]);
    if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
  }
  for (let i = n - 1; i >= 0; i--) {
    const x = xAt(i), y = yAt(p25[i]);
    ctx.lineTo(x, y);
  }
  ctx.closePath();
  ctx.fill();
  ...
  // slope arrow from left (0) to right (end)
  const yStart = yAt(0);
  const xStart = X0;
  const xEnd = X1;
  const yEnd = yAt(med[med.length - 1]);

  const bias = (meta.bias || "").toUpperCase();
  let arrowColor = "#e5e7eb";
  if (bias === "BULL") arrowColor = "#22c55e";
  else if (bias === "BEAR") arrowColor = "#f97373";
  ...
  // angle label (use meta.angle_deg if available)
  const label = typeof meta.angle_deg === "number" ? `${meta.angle_deg}°` : "";
  if (label) {
